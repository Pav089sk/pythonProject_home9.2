...
```get_mask_account```
5. Функция возврата информации по выполненным или не выполненным операциям
```filter_by_state```
...
## Тестирования 
Тесты выполнены для функций с применением фикстур, параметризации, mock и patch.
...
Для запуска тестирования через pytest необходимо воспользоваться командой ```pytest``` в терминале
...
## Lобавлен модуль Search
В модуль добавлены две функции
...
строка.
## Итоговый функционал
В модуле ```main.py``` реализован итоговый функционал всего приложения для работы с банковскими операциями.
Можно выбрать тип файла из которого мы берём данные и отфильтровать список транзакций по необходимым параметрам.