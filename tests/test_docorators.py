...
def test_log_1(capsys: CaptureFixture):
    ...
    log()(my_function)('1', 2)
    captured = capsys.readouterr()
    assert captured.out == ('my_function, "TypeError" can only concatenate str (not "int") to str, '
 "Inputs: ('1', 2), {}\n")
...
def test_log_2():
    ...
    with open('mylog.txt', 'r') as file:
        a = file.readlines()
        assert a[-1] == ('my_function,"TypeError" can only concatenate str (not "int") to str, '
 "Inputs: ('1', 2), {}\n")
